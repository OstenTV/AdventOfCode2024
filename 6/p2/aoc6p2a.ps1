...
for ($tryX = 0; $tryX -lt $Map.Length; $tryX++) {
    for ($TryY = 0; $TryY -lt $Map[$tryX].Length; $TryY++) {  # Corrected typo
        ...
        $Map = Clone-Map -map $SaveGame # Restore the map
        $Direction = 0
        $Counter = 0
        $Inside = $true
        $Position = $StartingPos
        $loopObstacleDirection = @()

        $sx = $StartingPos[0]
        $sy = $StartingPos[1]

        if ($Map[$tryX][$TryY] -ne $Guard -and ( !($completedMap) -or $completedMap[$tryX][$TryY] -eq $Used)) {
        ...
        while ($Inside) {
            ...
            if ($newY -ge 0 -and $newY -lt $Map[$newX].Length -and $newX -ge 0 -and $newX -lt $Map.Length) {
        
                if ($Map[$newX][$newY] -eq $Obstacle) {
                    
                    if ("$Direction-$newX-$newY" -in $loopObstacleDirection) {
                        # Means we are stuck in a loop
                        $Looping = $true
                        break
                    } else {
                        $loopObstacleDirection += "$Direction-$newx-$newY"
                        $Direction++
        ...
        if ($tryX -eq 0 -and $TryY -eq 0) {
            $completedMap = Clone-Map -map $Map
        }

        cls
        if ($Looping) {
            [Console]::WriteLine("Loop detected at position ($tryX, $TryY)")
            $LoopCounter++
        } else {
            [Console]::WriteLine("No Loop detected at position ($tryX, $TryY)")
        }
        $Map | % {[Console]::WriteLine("$($_ -join '')")}
        [Console]::WriteLine("")
